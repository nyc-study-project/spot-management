...
6. POST Requests
    - POST /studyspots returns 201 Created, a location header pointing to the newly created resource, and a link to the geocode to obtain the latitude, longitude, and postal code.
...
7. Linked Data
    ...
    - Post calls include a link to the geocode, for obtaining the latitude, longitude, and postal code of the study spot.

8. Asynchronous Geocoding
    - Implemented an asynchronous operation (POST /studyspots/{id}/geocode) that triggers Google Geocoding API to update the study spot’s coordinates.
